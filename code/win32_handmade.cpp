	...
	if (Library)
	{
		...
		if (SUCCEEDED(DirectSoundCreate(0, &DirectSound, 0)))
		{
...

			{
				...
				if (SUCCEEDED(DirectSound->CreateSoundBuffer(
						&BufferDesc, &GlobalSoundBuffer, 0)))
				{
								   ...
								   DWORD LockOffset, DWORD BytesToLock)
{
	...
	if (SUCCEEDED(GlobalSoundBuffer->Lock(LockOffset, BytesToLock, &Region1,
...
internal void RenderWeirdGradeint(win32_offscreen_buffer *Buffer, int XOffset,
								  int YOffset)
{
...
internal void Win32UpdateWindow(HDC DeviceContext, int WindowWidth,
								int WindowHeight,
					 ...
					 int ShowCmd)
{
	...
	if (RegisterClass(&WindowClass))
	{
		HWND Window = CreateWindowEx(
			0, "HandmadeHeroWindowClass", "Handmade Hero",
		...
		if (Window)
		{
			...
			while (GlobalRunning)
			{
				...
				RenderWeirdGradeint(&GlobalBackBuffer, GlobalXOffset,
									GlobalYOffset);
				...
				Win32UpdateWindow(DeviceContext, Dimension.Width,
				...
				int64 CounterElapsed =
					CurrentCounter.QuadPart - LastCounter.QuadPart;
				uint64 CycleElapsed = CurrentCycleCounter - LastCycleCounter;

				real32 MSPerFrame = 1000.0f * (real32)CounterElapsed /
									(real32)CounterPerSecond.QuadPart;
				real32 FPS =
					(real32)CounterPerSecond.QuadPart / (real32)CounterElapsed;
